---
description: Pull each submodule up to its upstream default branch, then verify tower-protocol lockstep across firmware, cli & hil
argument-hint: "[firmware|cli|protocol|jolt|hil] [--check]"
allowed-tools: Bash(git submodule:*), Bash(git -C:*), Bash(git fetch:*), Bash(git log:*), Bash(git status:*), Bash(git switch:*), Bash(git merge:*), Bash(git describe:*), Bash(git tag:*), Bash(grep:*), Bash(cat:*), Bash(python3:*), Read
---
...
same tag in `firmware`, `cli`, and `hil`**. The wire format is postcard (non
self-describing): if the consumers reference different protocol tags, frames are
...
For each of `protocol`, `jolt`, `cli`, `firmware`, `hil` (this order — protocol and
jolt are the dependencies):
```bash
...
This is the gate. Run the deterministic check (tag pins across firmware ×2 / cli / hil
**and** the resolved lockfile SHAs — a re-cut tag passes tag checks while building
different code):
```bash
...
- **script verdict** — a non-zero exit is a lockstep break. Flag it loudly and explain
  that this causes silent decode failures.
- **pinned tag vs latest protocol release** — if the consumers pin an older tag than
  the latest protocol release, note that an upgrade is available (and that it must be
  applied to *all* consumers together, then `cargo update -p tower-protocol` in each).
...
`cli` and `hil` also pin `jolt` by tag (the script above prints both). This is **not** a
lockstep gate — a `jolt` mismatch is a **compile error** in the consumer, not a silent
failure — but if the `jolt` submodule advanced past the pinned tags, note that an
